...
export default function PetDetail() {
  ...
  if (!pet) return <div style={{ textAlign: 'center', fontSize: '1.25rem', padding: '2rem' }}>Pet not found!</div>;
    ...
    <div>
      ...
      <div className="pet-image-section">
        ...
        <div className="pet-buttons">
          <button className="btn add-cart"  onClick={() => addToCart(pet)}>Add to Cart</button>
      ...
      <div className="pet-info-section">
        ...
        <div className="pet-diet-section">
          <h2>Diet</h2>
          <ul>
            {pet.diet.map((item, idx) => <li key={idx}>{item}</li>)}
          </ul>
        </div>
        ...
        <div className="pet-care-section">
          ...
          <ul>
            {pet.careInstructions.map((item, idx) => <li key={idx}>{item}</li>)}
          </ul>
        </div>
        ...
        <div className="pet-inclusions-section">
          <h2>Inclusions</h2>
          <ul>
            {pet.inclusions.map((item, idx) => <li key={idx}>{item}</li>)}
          </ul>
        </div>
        ...
        <div className="pet-price-section">
          ...
          {pet.originalPrice && (
            <span className="original-price">₹{pet.originalPrice.toLocaleString()}</span>
          )}
        </div>
      </div>
    </div>
